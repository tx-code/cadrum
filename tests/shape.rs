...
fn test_clone_is_independent() {
	...
	let original = test_box();
	let copy = original.clone();
	let other: Vec<Solid> = vec![Solid::box_from_corners(
		dvec3(5.0, 5.0, 5.0),
...
fn test_translated_preserves_volume() {
	...
	let moved: Vec<Solid> = shape
		.into_iter()
		.map(|s| s.translate(dvec3(100.0, 200.0, -50.0)))
		.collect();
...
fn test_translated_preserves_shell_count() {
	...
	let moved: Vec<Solid> = shape
		.into_iter()
		.map(|s| s.translate(dvec3(5.0, 0.0, 0.0)))
		.collect();
...
fn test_union_of_translated_overlapping_solids_has_single_volume() {
	...
	let a = vec![Solid::box_from_corners(
		dvec3(0.0, 0.0, 0.0),
		dvec3(10.0, 10.0, 10.0),
	)];
	let b = vec![Solid::box_from_corners(
		dvec3(100.0, 0.0, 0.0),
		dvec3(110.0, 10.0, 10.0),
	)];
	let b_moved: Vec<Solid> = b
		.clone()
		.into_iter()
		.map(|s| s.translate(dvec3(-100.0, 0.0, 0.0)))
		.collect();
	...
	assert!(
		(volume_no_move - 2000.0).abs() < 1e-3,
		"expected volume ~2000, got {volume_no_move}"
	);
	...
	assert!(
		...
		"expected volume ~1000, got {volume}"
	);
	...
	assert!(
		(volume_with_b - 2000.0).abs() < 1e-3,
		"expected volume ~2000, got {volume_with_b}"
	);
}
...
fn test_rotated_preserves_volume() {
	let shape = test_box();
	// Z 軸周りに 45° 回転
	let rotated: Vec<Solid> = shape
		.into_iter()
		.map(|s| s.rotate(DVec3::ZERO, DVec3::Z, std::f64::consts::FRAC_PI_4))
		.collect();
...
fn test_rotated_full_turn_preserves_volume() {
	...
	let rotated: Vec<Solid> = shape
		.into_iter()
		.map(|s| s.rotate(DVec3::ZERO, DVec3::Z, std::f64::consts::TAU))
		.collect();
	assert!((rotated.iter().map(|s| s.volume()).sum::<f64>() - 1000.0).abs() < 1e-3);
}
...
fn test_rotated_preserves_shell_count() {
	...
	let rotated: Vec<Solid> = shape
		.into_iter()
		.map(|s| s.rotate(DVec3::ZERO, DVec3::Y, std::f64::consts::FRAC_PI_2))
		.collect();
...
fn test_preserves_face_ids() {
	...
	let moved: Vec<Solid> = shape
		.into_iter()
		.map(|s| s.translate(dvec3(10.0, 0.0, 0.0)))
		.collect();
	assert_eq!(
		solid_id,
		moved[0].tshape_id(),
		"translate should preserve solid tshape_id"
	);
	...
	assert_eq!(
		solid_id,
		rotated[0].tshape_id(),
		"rotate should preserve solid tshape_id"
	);
...
fn test_mirrored_octants_union_volume_is_eight() {
	...
	let bx: Vec<Solid> = b
		.iter()
		.map(|s| s.mirrored(DVec3::ZERO, DVec3::X))
		.collect();
	let by: Vec<Solid> = b
		.iter()
		.map(|s| s.mirrored(DVec3::ZERO, DVec3::Y))
		.collect();
	let bz: Vec<Solid> = b
		.iter()
		.map(|s| s.mirrored(DVec3::ZERO, DVec3::Z))
		.collect();
	let bxy: Vec<Solid> = bx
		.iter()
		.map(|s| s.mirrored(DVec3::ZERO, DVec3::Y))
		.collect();
	let bxz: Vec<Solid> = bx
		.iter()
		.map(|s| s.mirrored(DVec3::ZERO, DVec3::Z))
		.collect();
	let byz: Vec<Solid> = by
		.iter()
		.map(|s| s.mirrored(DVec3::ZERO, DVec3::Z))
		.collect();
	let bxyz: Vec<Solid> = bxy
		.iter()
		.map(|s| s.mirrored(DVec3::ZERO, DVec3::Z))
		.collect();
	...
	for other in &octants[1..] {
		result = cadrum::Boolean::union(&result, other)
			.expect("union failed")
			.into();
	}
	let volume: f64 = result.iter().map(|s| s.volume()).sum();
...
fn test_scaled_union_with_original_volume_is_nine() {
	...
	let result: Vec<Solid> = cadrum::Boolean::union(&b, &b_scaled)
		.expect("union failed")
		.into();
	let volume: f64 = result.iter().map(|s| s.volume()).sum();
	assert!(
		(volume - 9.0).abs() < 1e-3,
		"expected volume ~9 (1 + 8), got {volume}"
	);
}
...
fn test_vec_solid_roundtrip() {
	...
	assert!(
		(sum - total_volume).abs() < 1e-6,
		"sum={sum}, expected={total_volume}"
	);
}
...
fn test_new_faces_subtract_b_inside_a() {
	...
	let big: Vec<Solid> = vec![Solid::box_from_corners(
		dvec3(0.0, 0.0, 0.0),
		dvec3(10.0, 10.0, 10.0),
	)];
	let small: Vec<Solid> = vec![Solid::box_from_corners(
		dvec3(3.0, 3.0, 3.0),
		dvec3(7.0, 7.0, 7.0),
	)];
...
fn test_new_faces_intersect_b_inside_a() {
	...
	assert_eq!(
		result.solids().iter().flat_map(|s| s.face_iter()).count(),
		tool_count,
...
fn test_bounding_box() {
	// 単一 solid
	let [min, max] =
		Solid::box_from_corners(dvec3(1.0, 2.0, 3.0), dvec3(4.0, 6.0, 8.0)).bounding_box();
	...
	// translate 後に追従する
	let moved: Vec<Solid> = test_box()
		.into_iter()
		.map(|s| s.translate(dvec3(10.0, 20.0, 30.0)))
		.collect();
